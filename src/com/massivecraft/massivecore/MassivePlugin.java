...
public abstract class MassivePlugin extends JavaPlugin implements Listener, Named
{
	...
	{
		return getClassesActive("cmd", MassiveCommand.class, new Predicate<Class<?>>()
			{
				@Override
				public boolean apply(Class<?> clazz)
				{
					try
					{
						// If the method get is present it is probably a singleton
						// We can not get the singleton however, since it might not be present
						// before other things have been instantiated.
						ReflectionUtil.getMethod(clazz, "get");
						return true;
					}
					catch (Exception ex)
					{
	...
	{
		return getClassesActive("entity.migrator", Migrator.class);
	}

	public List<Class<?>> getClassesActive(Class<? extends Active> superClass, Predicate<Class<?>>... predicates)
	{
		return getClassesActive(superClass.getSimpleName().toLowerCase(), superClass, predicates);
	}
	...
	public List<Class<?>> getClassesActive(String packageName, final Class<?> superClass, Predicate<Class<?>>... predicates)
	{
		...
		return ReflectionUtil.getPackageClasses(packageName, this.getClassLoader(), true, predicateCombined, predicateNotAbstract, predicateSubclass);
	}
