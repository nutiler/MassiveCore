...
public abstract class CommandEditContainerAbstract<O, V> extends CommandEditAbstract<O, V>
{
	...
	public void attemptSetPerform(V after)
	{
		...
		{
			messages.add(Mson.prepondfix(mson("Additions:").color(ChatColor.RED), this.getValueType().getVisualMson((V) additions, sender), null));
		}
		...
		{
			messages.add(Mson.prepondfix(mson("Deletions:").color(ChatColor.RED), this.getValueType().getVisualMson((V) deletions, sender), null));
		}
